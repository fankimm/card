...
// 이 앱에서 유일한 쓰기 경로다.
//
// 예전 주소는 /api/hello 였다(create-next-app 기본 파일에 그대로 얹은 게 굳었다).
// 지금은 별칭으로 남겨 뒀고, 그쪽으로 들어온 요청은 로그 비고에 '(구주소)' 를 붙여
// 표시한다. 세 명 다 새 주소로 넘어간 게 로그에서 확인되면 별칭을 지우면 된다.
...
export default async function handler(
  ...
  const 구주소 = (req.url || '').split('?')[0].endsWith('/hello');
...
async function 처리(
  ...
  if (parsed.kind === '무시') {
    await 로그남기기({ 결과: '무시', 원문, 비고: parsed.reason, 구주소 });

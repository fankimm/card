...
**현대카드가 문자 양식을 바꾼다**
예고 없이 바뀐다(2026-07, 2026-08, 2026-09-16 세 번). 바뀌면 점심 문자가 로그 탭에
`무시 / 결제 문자가 아닙니다 (...)` 로 쌓이고 시트에는 아무것도 안 들어간다.
`lib/sms-parse.ts` 는 그래서 줄 번호가 아니라 줄의 모양(날짜 꼴·금액 꼴)으로 읽는다.
놓친 문자는 파서를 고쳐 배포한 뒤 로그 탭의 원문을 그대로 다시 POST 하면 복구된다.

```bash
curl -X POST https://card-usages.vercel.app/api/usages/ingest -H 'content-type: application/json' -d '{"test":"<로그 탭 원문>"}'
```
